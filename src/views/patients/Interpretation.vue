  ...
  <div @click="handleHideDialog">
    ...
    <div class="query-details">
      <div class="section-tip">
        用时 <span>{{ time }}</span> s，检索了 <span>16,257</span> 个基因， <span>9,014</span>个疾病 ，<span>12,568,986</span> 个变异后，发现了高度关注 <span>{{ sectionTip.gdNum }}</span>个 ,中度关注 <span>{{ sectionTip.zdNum }}</span>个, 其他变异 <span>{{ sectionTip.otherNum }}</span>个
      </div>
      <!-- 高度关注 -->
                    ...
                    if (data) {
                        ...
                        Object.keys(this.highAttention).forEach(item => {
                            ...
                            if (this.highAttention[item]['wenXian']) {
                                ...
                                this.highAttention[item]['weiXianKey'] =
                                    weiXianKeyArr[0] +
                                    '-' +
                                    weiXianKeyArr[weiXianKeyArr.length - 1]
                                this.highAttention[item][
                                    'weiXianKeys'
                                ] = weiXianKeyArrs
                            }
                        })
                        ...
                        Object.keys(this.middleAttention).forEach(item => {
                            ...
                            if (this.middleAttention[item]['wenXian']) {
                                Object.keys(
                                    ...
                                    let obj = {
                                        name: this.middleAttention[item][
                                            'wenXian'
                                        ][key],
                                    }
        ...
        concatHandle() {
            ...
            let highAttentionObj = Object.keys(this.highAttention).map(item => {
                ...
                if (this.highAttention[item]['weiXianKeys']) {
                    highAttentionItem = this.highAttention[item][
                        'weiXianKeys'
                    ].map(fItem => fItem.name)
                }
            ...
            let middleAttentionObj = Object.keys(this.middleAttention).map(
                item => {
                    ...
                    if (this.middleAttention[item]['weiXianKeys']) {
                        middleAttentionItem = this.middleAttention[item][
                            'weiXianKeys'
                        ].map(fItem => fItem.name)
                    }
